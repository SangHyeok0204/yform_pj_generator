...
SLIDE_3_5 = Slide(
    ...
    blocks=[
        H("이론·실증 → 설계 결정 매핑"),
        P("아래 6개 설계 결정은 모두 본 절에서 다룬 세 논문의 결론을 직접 반영한다. 다섯 가지는 구현 완료, 동적 σ만 논의 중이다."),
        G(6),
        LI("Net Delta 재고 — S&S Thm.2 (이론) + Muravyev OMM delta exposure 관리 관행 (실증). 구현 완료.", num="1."),
        LI("다중 호가 3×3 — A-S 한계 ②(단일 호가) 직접 해결 + 실제 LOB 구조 반영. 구현 완료.", num="2."),
        LI("방향성 Bucket 체결 확률 — A-S 한계 ③(단방향 체결) 해결 + Muravyev 방향성 비대칭 실증 반영. 구현 완료.", num="3."),
        LI("즉시 체결 + mid price — A-S 한계 ③ 보강. δ≤0 시 즉시 체결 + mid 집행으로 누적 재고 즉시 해소. 구현 완료.", num="4."),
        LI("T = 당일 장 마감 — A-S 한계 ④ + Muravyev 오버나이트 재고 0.02% 실증. 구현 완료.", num="5."),
        LI("동적 σ — A-S 구조적 한계 + S&S 장 막판 민감도 이론. 방안 A·B·C 중 선택 논의 중 (미정).", num="6."),
    ],
)
